...
# Бренд стартовал позже начала окна: считать его средние по дням, когда товара
# ещё не было в продаже, значит занижать всё вдвое. Обрезаем период и говорим
# в заголовке, с какого дня он на самом деле считается.
if START_DATE:
    MON = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля",
           "августа", "сентября", "октября", "ноября", "декабря"]
    cut = []
    for k, lb, s_, e_, p_ in PERIODS:
        if s_ < START_DATE:
            s_ = START_DATE
            if k in ("d14", "d28"):
                lb = f"с {START_DATE.day} {MON[START_DATE.month - 1]}"
        cut.append((k, lb, s_, e_, p_))
    # если после обрезки два периода совпали по датам — оставляем один
    PERIODS, seen = [], set()
    for k, lb, s_, e_, p_ in cut:
        sig = (s_, e_)
        if k in ("d14", "d28") and sig in seen:
            continue
        seen.add(sig)
        PERIODS.append((k, lb, s_, e_, p_))
...
def assembly(cab):
    ...
    for o in orders:
        ...
        if not d:
            continue
        rows.append(dict(day=d, state=cls(o), wh=o.get("warehouseId"),
    ...
    # ------------------------------------------------ скорость отгрузки
    # Считается одинаково для всего кабинета и для каждого склада продавца
    # отдельно: складов бывает несколько, и отгружают они по-разному.
    sup = {x["id"]: x for x in (mp.get("supplies") or [])}
    now_utc = datetime.datetime.now(datetime.timezone.utc)

    def speed_block(sel_orders, sel_rows):
        ...
        made = collections.Counter(r["day"] for r in sel_rows)
        ship_ok = collections.Counter(r["day"] for r in sel_rows if r["state"] == "shipped")
        pend = collections.Counter(r["day"] for r in sel_rows if r["state"] == "assembling")
        canc = collections.Counter(r["day"] for r in sel_rows if r["state"] in ("cancel", "defect"))
        days = []
        for d in sorted(set(by_day) | set(made)):
            a = by_day[d]
            days.append(dict(date=d, created=made.get(d, 0), shipped=ship_ok.get(d, 0),
        ...
        t_qty = sum(x["qty"] for x in days)
        t_sum = sum(x["sum"] for x in days)
        t_delta = (sum(by_day[d]["delta_w"] for d in by_day) / t_sum) if t_sum else None
        return dict(
            days=days, created=sum(x["created"] for x in days),
            shipped=sum(x["shipped"] for x in days),
            qty=t_qty, sum=round(t_sum, 2),
            buckets=[sum(x["buckets"][i] for x in days) for i in range(len(SPEED))],
            delta=round(t_delta, 3) if t_delta is not None else None,
            best_delta=BEST_DELTA,
            gap=round(abs(BEST_DELTA - t_delta), 3) if t_delta is not None else None,
            gap_money=round(abs(BEST_DELTA - t_delta) / 100 * t_sum, 2) if t_delta is not None else None,
            ...
            basis=SPEED_BASIS,
            fallback_qty=fb_qty,
            fallback_share=round(fb_qty / t_qty, 3) if t_qty else 0,
        )

    speed = speed_block(orders, rows)
    wh_names = {w.get("id"): (w.get("name") or str(w.get("id")))
                for w in (mp.get("warehouses") or []) if w.get("id")}
    used = collections.Counter(r["wh"] for r in rows if r.get("wh"))
    speed_wh, wh_list = {}, []
    for wid, n in used.most_common():
        sel_o = [o for o in orders if o.get("warehouseId") == wid]
        sel_r = [r for r in rows if r.get("wh") == wid]
        speed_wh[str(wid)] = speed_block(sel_o, sel_r)
        wh_list.append(dict(id=str(wid), name=wh_names.get(wid, f"склад {wid}"),
                            cab=cab, tasks=n))
...
def fbw_plateau(blocks):
    ...
    for b in blocks:
        ...
        for i, d in enumerate(b["days"]):
            ...
            t[0] += b["o"][i]
            t[1] += b["b"][i]
    curve = [(a, v[1] / v[0]) for a, v in sorted(tot.items()) if v[0] >= 30]
    if not curve:
        return None
...
def cabinet(cab):
    ...
    elif mat_raw >= MIN_MATURE:
        buyout = mat_buy / mat_raw
        bsrc = f"дозревшие когорты FBS: выкуплено {mat_buy} из {mat_raw} заказов старше {MATURITY} дн."
        bproxy = False
    elif fin_buyout and proxy_ok:
        buyout = fin_buyout
        w = (st.get("cohort_whole") or {}).get("window", "")
        bsrc = (f"прокси: выкуп кабинета целиком за {w}. Дозревших когорт FBS пока "
                f"{mat_raw} из нужных {MIN_MATURE}")
        bproxy = True
    else:
    ...
    if payout is None:
        ...
        if own and own.get("qty", 0) >= 30:
            ...
            payout_src = f"{own.get('tag') or 'FBS этого кабинета'}, {own['qty']} шт продаж"
        else:
    ...
    def ads_for(dd):
        ...
        share = fbsv / base if base else 0
        return total_spend * share, share, total_spend, \
            "по доле FBS в заказах кабинета (детализации по nmId нет)"
    ...
    for k, label, s, e, partial in PERIODS:
        ...
        fwd = st.get("logistics_fwd_per_order")
        back = st.get("logistics_back_per_order")
    ...
    return dict(
        ...
        rates=dict(
            ...
            mature_raw=mat_raw, mature_bought=mat_buy, maturity_days=MATURITY,
        ...
        warehouses=sorted({r.get("warehouseName", "") for r in fo}),
...
if ASM:
    ...
    asm_total["warehouses"] = [w for a in ASM.values() for w in (a.get("warehouses") or [])]
    asm_total["speed_wh"] = {k: v for a in ASM.values()
                             for k, v in (a.get("speed_wh") or {}).items()}
